...
def _thermal_verdict(db: Session, node: models.Node, now: datetime):
    ...
    return schemas.ThermalHealthResponse(
...
def get_node_health(node_id: int, db: Session = Depends(get_db),
    ...
    return schemas.NodeHealthResponse(
        ...
        thermal=_thermal_verdict(db, node, now),
    )
